...
func TestMainHandlerWhenCountMoreThanTotal(t *testing.T) {
	totalCount := 4
	req := httptest.NewRequest("GET", "/cafe?count="+strconv.Itoa(totalCount+1)+"&city=moscow", nil) // здесь нужно создать запрос к сервису
	...
	assert.Equal(t, expectedCafes, responseRecorder.Body.String()) // Проверяем все доступные кафе
}
